...
日期：2026-09-12。初始实现与 CPU smoke 已完成，后续 A100 校准也已通过。用户现已授权接入完整目录的多样 304 边代表并提交一次单卡试验；最新入口与预算见[单卡试验](corpus-single-gpu-pilot.md)。以下初始实现细节与历史 smoke 记录保留供追溯。

**当前结论：流程能够运行，尚未证明它比纯局部搜索更有效，也没有得到 305 边构造。** CUDA/bf16 在 A100 80GB 上已实测；当前正式配置的完整试跑外推约 19 分钟，尚不是完整训练实测。新的初始训练池为 180 个经轨道审核的 304 边代表。
