...
最新进度：[第二次训练方案](graphgps-pilot-diagnostic-plan.md)已实现，21 项测试与两组 CPU smoke 通过。作业 **21929229 已在 g-10-01 的单 A100 80GB 开始运行**，顺序比较参考池 80% 权重和均匀回填，各三个种子；已观察首组训练到第 1,000 步。此处旧作业结果保留作基准，新作业的实际目录见方案及提交回执。
...
**首次试验结论：完整流程能够运行，但模型原始生成最高 286 边，局部修复后最高 290 边，未改善初始训练集的 304 边，也没有得到 305。** 单 A100 80GB 实测 19 分 39 秒、11,000 训练步、12,288 张修复后候选，全部保存的候选均经本地独立整数复核。初始 loss 下降；精英回填使 304 边样本的训练占比从 100% 降为约 35%，这是第二次试验重点检验的问题。
